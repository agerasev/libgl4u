...
void Program::attach(Shader *s) throw(Exception) {
	if(s == nullptr)
...
void Program::detach(Shader *s) throw(Exception) {
	glDetachShader(_id, s->id());
	_shaders.remove(s);
	_updateVariables();
}

void Program::link() throw(Exception) {
	int link_ok;
	glLinkProgram(_id);
...
void Program::_updateVariables() throw(Exception) {
	_attribs.clear();
	_uniforms.clear();
	for(Shader *s : _shaders) {
		for(int i = 0; i < 2; ++i) {
...
void Program::_getVariablesID() throw(Exception) {
...
const std::map<std::string, Program::AttribVariable> &Program::attributes() const {
	return _attribs;
}
const std::map<std::string, Program::UniformVariable> &Program::uniforms() const {
	return _uniforms;
}

void Program::_loadUniform(const UniformVariable &var) {
...
void Program::setUniform(const std::string &name, const Texture *tex) {
